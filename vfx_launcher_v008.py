...
class GetNodeWindow(QWidget):
    ...
    def load_nodes(self):
        ...
        if not houdini_path:
            QMessageBox.warning(self, "Error", "Houdini path is not set.")
            return
        ...
        if not os.path.exists(hython_path):
            QMessageBox.warning(self, "Error", f"hython.exe not found at {hython_path}. Check your Houdini installation.")
            return

        # Create temp script using write() to avoid indentation issues
        ...
        if not nodes:
            QMessageBox.warning(self, "Error", "No nodes found in the Houdini file.")
    ...
    def load_parameters(self):
        ...
        temp_script.write("params = [parm.name() for parm in node.parms()]\n")
        temp_script.write("print('PARAM_LIST_START')\n")
        temp_script.write("for p in params:\n")
        temp_script.write("    print(p)\n")
